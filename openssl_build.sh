...
ANDROID_NDK_PATH=${WORK_PATH}/android-ndk-r23b
OPENSSL_SOURCES_PATH=${WORK_PATH}/openssl-1.1.1p
ANDROID_TARGET_API=$1
ANDROID_TARGET_ABI=$2
OUTPUT_PATH=${WORK_PATH}/openssl_1.1.1p_${ANDROID_TARGET_ABI}
...
then
    ...
    PATH=$ANDROID_NDK_HOME/toolchains/llvm/prebuilt/linux-x86_64/bin:$PATH
    cd ${OPENSSL_TMP_FOLDER}
    ./Configure android-arm64 -DANDROID_API=${ANDROID_TARGET_API}  shared threads no-asm no-sse2 no-ssl2 no-ssl3 no-comp no-hw no-engine --prefix=${OUTPUT_PATH}
    build_library
